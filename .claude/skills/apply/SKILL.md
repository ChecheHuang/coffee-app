...
用 **AskUserQuestion**（單選）：
- "繼續 Phase B — 設計同步（推薦）"
- "暫停，稍後繼續 Phase B"
- "中止 apply"

選「暫停」或「中止」則結束本 session，告知使用者下次跑 `/apply <name>` 可從 Phase B 繼續。
...
選「暫停」或「中止」則結束本 session，告知使用者下次跑 `/apply <name>` 可從 Phase C 繼續。
...
2. **[必做] 切 mobile viewport**：
   mcp__chrome-devtools__emulate({ viewport: "375x812x3,mobile,touch" })
   — iPhone 13/14 尺寸 + mobile + touch flags。
   若 Chrome 已有 about:blank 或其他 page，也要立即 emulate，不要在桌面寬度下測。
   桌面寬度會讓 flex/grid 佈局「剛好」看起來正確，掩蓋手機上的斷行、溢位、元素重疊問題。
3. mcp__chrome-devtools__navigate_page 到 http://localhost:8081/<affected page>，確認頁面可載入
4. 用 list_console_messages 掃一次 baseline error，記下哪些是 pre-existing（後續驗證可忽略）
```

**這個 session 在整個 /apply 中保活**，不要每個 task 都關開。

**viewport 一旦設定，整個 /apply session 期間保持**——就算中途因為 navigate_page 重新載入頁面，emulate 的 viewport 設定不會遺失。
...
- **[必做] Chrome 一啟動立刻切 mobile viewport**：`emulate({ viewport: "375x812x3,mobile,touch" })`。這是手機 app 專案，桌面寬度測試會掩蓋佈局 bug（flex 溢位、斷行、元素重疊在手機上才會出現）。若意識到忘了切，立即補 emulate + 重新 navigate/reload 該頁
